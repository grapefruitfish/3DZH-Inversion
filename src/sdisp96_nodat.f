! "nodat" means the cp,t,mode,ifunc are replace by pointer.
! with pointer,the tmp ray file can now be remove, so sregn_sub can
! run concurrently.
      subroutine sdisp96_nodat(dt,npts,hs,hr,faclov,facray,mode,
     & dolove,dorayl,fval,nfval,mmax,iflsph,refdep,cp_ptr,t_ptr,
     & mode_ptr,ifunc_ptr,allfluid,d,a,b,rho)
        implicit none
        !implicit double precision (a-h,o-z)
        !interface
            !subroutine disprs(ilvry,dt,nn,iret,mname,
            !1    verby,nfval,fval,ccmin,ccmax,
            !2    ifunc_ptr,mode_ptr,t_ptr,cp_ptr)
                !integer,pointer::ifunc_ptr(:),mode_ptr(:)
                !real*8,pointer::t_ptr(:)
                !real*8,pointer::cp_ptr(:,:)
                !character mname*120
                !logical verby
                !integer nfval,iret,nn,ilvry
                !real fval(2049)
                !real*4 ccmin,ccmax,dt
            !endsubroutine
        !endinterface
...
c        integer istat
        ...
        ! LIN=5 is the stdin; LOT=6 is the stdout. ruoshan
        integer LIN, LOT, LER, NL, NL2
        parameter (LER=0, LIN=5, LOT=6, NL=200, NL2=NL+NL)
c        equivalence(vth(1),vts(1,1)),(vtp(1),vts(1,2))
c        real*4 vth(NL2),vtp(NL2)

c        common/model_sdisp/ d,a,b,rho
c        real*4 d(NL),a(NL),b(NL),rho(NL)

c        common/pari_sdisp/ mmax,mode ! commented by Ping
        integer mmax, mode

c        common/water_sdisp/iwat(NL) ! commented by Ping
c        integer iwat

c        common/pard_sdisp/ twopi,displ,dispr commented by Ping
        double precision displ,dispr,twopi

c        common/vels_sdisp/ mvts(2),vts(NL2,2) !commented by Ping
c        integer mvts(2)
c        real*4 vts(NL2,2)
        ...
        !ruoshan,the implicit variables
        double precision faclov,facray,hs,hr
        double precision xx
        integer i
        integer idimen,icnvel,ierr
        integer idispl,idispr
        integer iunit,iiso,iflsph
        integer ilvry,iret
        integer l
        integer n1,n2,npts
!-----    Replace the pointer stuffs with common.For the sake of Fortran
c        common/rs_ptr_shared/rs_uruz(4,NL),cp_ptr(MAXMOD,NL),t_ptr(NL),
c     1            mode_ptr(NL),ifunc_ptr(NL)
c        real*8 rs_uruz,t_ptr,cp_ptr
        integer ifunc_ptr(NL),mode_ptr(NL)
        real*8 t_ptr(NL),cp_ptr(MAXMOD,NL)
...
c        common/isomod/od(NL),oa(NL),ob(NL),orho(NL),
c     1      qa(NL),qb(NL),etap(NL),etas(NL), 
c     2      frefp(NL), frefs(NL)
        real*4 od(NL), oa(NL), ob(NL), orho(NL)
...
c-----
c        character mname*120
        logical dolove, dorayl
c-----
c       model characterization
c-----
c        common/modspec_sdisp/allfluid commented by Ping
        logical allfluid
c        real kappa, mu
c-----
...
c-----
        call gcmdln_sdisp(verby,ccmin,ccmax) 
c-----
c       main program just does the control.
c-----
        twopi=6.283185307179586d+00

        !ruoshan:use dynamic array to replace the tmp ray file. well,at
        !least it won't hurt. :)
!       allocate(ifunc_ptr(npts),STAT=istat)
!       print *,istat
!       allocate(mode_ptr(npts),STAT=istat)
!       print *,istat
!       allocate(t_ptr(npts),STAT=istat)
!       print *,istat
!       allocate(cp_ptr(MAXMOD,npts),STAT=istat)  !for cache,but bad
!       print *,istat
        ifunc_ptr(:)=-1
        mode_ptr(:)=-1
...
c        mode = nmodes ! commented by Ping
c-----
...
!           endif
!1000   continue
!!! only for Raleigh wave case
        ilvry=2
        call disprs(ilvry,dt,npts,
     1   verby,nfval,fval,ccmin,ccmax,mmax,iflsph,displ,dispr,
     1   allfluid,mode,refdep,cp_ptr,t_ptr,ifunc_ptr,mode_ptr,d,a,b,rho)
        end !main

        subroutine mdsrch_sdisp(cmin,cmax,allfluid,mmax,mvts,vts,a,b)
c-----
...
c-----
        ...
        parameter(LER=0) !haha found this bug so late
        implicit double precision (a-h,o-z)
...
c        common/pari_sdisp/ mmax,mode
        integer mmax
c        common/water_sdisp/iwat(NL) ! commented by Ping
c        common/vels_sdisp/ mvts(2),vts ! commented by Ping
        integer mvts(2)
...
c-----
  ...
  310       continue
            call bsort_sdisp(m,vth,1)
            m=m+1
            vth(m)=1.0e+30
            mvts(1)=m

            vts(1:m,1)=vth ! added by Ping
  ...
  320       continue
            call bsort_sdisp(m,vtp,1)
            m=m+1
            vtp(m)=1.0e+30
            mvts(2)=m

            vts(1:m,2)=vtp ! added by Ping
c-----
...
c-----
            cmin = betmn
            call gtsolh_sdisp(a(jmn),b(jmn),cmin)
            cmin=.95*cmin
            cmax = betmx
        endif
c        WRITE(LER,*)'Searching for ',cmin, ' <= c <= ',cmax
c-----
...
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        function dltar_sdisp(wvno,omega,kk,allfluid,mmax,iwat,
     1                      d,a,b,rho)
c-----
...
c-----
            dltar_sdisp = dltar1_sdisp(wvno,omega,mmax,iwat,d,a,b,rho)
C            WRITE(6,*)'T,C,FL:',6.2831853/omega,omega/wvno,dltar_sdisp
C            DO I=1,10
C            dltar_sdisp = dltar1_sdisp(wvno-0.05*I*wvno,omega)
C            WRITE(6,*)'T,C,FL:',6.2831853/omega,
C     1         omega/(wvno-0.05*I*wvno),dltar_sdisp
C            ENDDO
...
c-----
            dltar_sdisp = dltar4_sdisp(wvno,omega,allfluid,mmax,iwat,
     1                                 d,a,b,rho)
C        WRITE(6,*)6.2831853/omega, omega/wvno,dltar_sdisp
        endif
        end

        function dltar1_sdisp(wvno,omega,mmax,iwat,d,a,b,rho)
c-----
...
c-----
        ...
        real*4 d(NL),a(NL),b(NL),rho(NL)
c        common/model_sdisp/ d,a,b,rho
c        common/pari_sdisp/ mmax,modew ! commented by Ping
        integer mmax
c        common/pard_sdisp/ twopi,displ,dispr commented by Ping
c        common/water_sdisp/iwat(NL) ! commented by Ping
        integer iwat(NL)
        ...
        logical lshimag
        !complex*16 rsh !ruoshan,dont know why using complex
...
c-----
        ...
        call gtegsh_sdisp(mmax,wvno, omega, rsh, lshimag,iwat,b)
        call gtesh_sdisp(esh,einvsh,rsh,wvno,mu,lshimag)
        ...
        do 600 m=mmm1,1,-1
            if(iwat(m).eq.0)then
               call gtegsh_sdisp(m,wvno, omega, rsh, lshimag,iwat,b)
               mu = rho(m)*b(m)*b(m)
               call gtesh_sdisp(esh,einvsh,rsh,wvno,mu,lshimag)
               call varsh_sdisp(d(m),rsh,lshimag,
     1                          cossh,rsinsh,sinshr,exm)
               call hskl_sdisp(cossh,rsinsh,sinshr,
     1                         mu,iwat(m),hl,exm,exe)
...
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        subroutine bsort_sdisp(nn,x,isign)
c       do bubble sort.
...
40      continue
50    continue
        nn=n-m  !consider this as a bug? ruoshan:nn=n-m
        return
        end
c
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        subroutine disprs(ilvry,dt,nn, !this is the only
                                    !subroutine I don't change the name.
     1          verby,nfval,fval,ccmin,ccmax,
     2          mmax,iflsph,displ,dispr,allfluid,mode,refdep,
     3          cp_ptr,t_ptr,ifunc_ptr,mode_ptr,od,oa,ob,orho)
...
c-----
        implicit double precision (a-h,o-z)
        integer NL, NL2,LIN,LER,LOT,NPERIOD,MAXMOD
        parameter (LER=0, LIN=5, LOT=6, NL=200,NL2=NL+NL)
        parameter(NPERIOD=80)
        real fval(NPERIOD)
        parameter(MAXMOD=1)
...
c        common/model_sdisp/ d,a,b,rho
        real*4 d(NL),a(NL),b(NL),rho(NL)
c        common/phas/ cp(MAXMOD)
        double precision cp(MAXMOD)
c        common/pari_sdisp/ mmax,mode ! commented by Ping
        integer mmax,mode
c        common/water_sdisp/iwat(NL)
        integer iwat(NL)
c        common/pard_sdisp/ twopi,displ,dispr commented by Ping
c        common/stor/ dcc(MAXMOD),c(MAXMOD),nlost,index,nroot1
        double precision dcc(MAXMOD),c(MAXMOD)
        integer nlost,index,nroot1

c        common/vels_sdisp/ mvts(2),vts
        integer mvts(2)
        real refdep
c        common/depref/refdep
c        common/isomod/od(NL),oa(NL),ob(NL),orho(NL),
...
c     2      frefp(NL), frefs(NL)
        ...
        real*4 fpar(10)
        integer*4 iflsph
c        integer*4 rs_iflsph
        logical allfluid
c        common/shared_mdl_left/rs_mmax,rs_iflsph
c-----
c       initialize
c-----
        data ipar/10*0/
...
c        iflsph=rs_iflsph
        nsph = iflsph
        ipar(1) = nsph
!TODO: this may cause problem --- 19:59 2012-11-13
!     WHY the getmod is different!!! shit!
        fpar(1) = refdep
        fpar(2) = refdep
c-----
...
c-----
        if(nsph.gt.0)then
            call sphere_sdisp(ilvry,mmax,d,a,b,rho)
            !write(LER,30)
            !write(LER,40)
c            if(nsph.eq.0)then
c                  WRITE(LER,*)'Velocity model is flat'
c            else
c                  WRITE(LER,*)'Velocity model is spherical. ',
c     1                'The following is the flattened model'
c            endif
c            do 341 i=1,mmax-1
c                !write(LER,10) d(i),a(i),b(i),rho(i)
c  341       continue
            !write(LER,20) a(mmax),b(mmax),rho(mmax)
        endif
c-----
c       look at the model, determined the range of velocities
c       for the SH or P-SV problems
c-----
        call mdsrch_sdisp(cmin,cmax,allfluid,mmax,mvts,vts,a,b)
       if(ccmin.gt.0.0)cmin = ccmin
       if(ccmax.gt.0.0)cmax = ccmax
c        write(LER,*)'cmin,cmax',cmin,cmax
c-----
c        if(ilvry.eq.1)then
c            write(LER,*)'Love Computations'
c        else if(ilvry.eq.2)then
c            write(LER,*)'Rayleigh Computations'
c        endif
c        write(LER,*) ' Number of layers=',mmax,' Maximum modes=',mode
c-----
c       read in control parameters.
c-----
c        iret = 1
c        write(LER,*) ' NPTS=',nn,' DT=',dt,' CMIN=',cmin, ' nfval=',
c     1     nfval
        df=1./(nn*dt)
        if(nfval.gt.0)then
            kmax = nfval
...
c       Rayleigh Wave
c-----
            !the output file. ruoshan,modify to formatted
!           open(1,file='/tmp/sdisp96.ray',status='unknown',
!    1          form='formatted', access='sequential')
            disp = dispr
        endif
!       rewind 1
        !print model information,ruoshan NOT comment out
c        call ptsmdl(1,mmaxx,d,a,b,rho,qa,qb,kmaxx,
c     1      ipar,fpar) !ruoshan,out 1
                             !filenumber:1 is ignored

c        call ptsmdl(d,a,b,rho)
c,kmaxx,ipar,fpar)
c-----
...
c---------------------------------------
  ...
  600   continue
        nroot1=MAXMOD
        rs_count=1
        do 2800 k = k1,kmax !label 2800 is the main loop,ruoshan
c---------------------------------------
c           search poles in F-K domain.
c---------------------------------------
            if(nfval.le.0)then
...
c-----
            kmode=0
            call poles_sdisp(ifunc,omega0,omega1,cmin,cmax,displ,dispr,
     1           allfluid,mmax,mode,dcc,c,nlost,index,nroot1,
     2           mvts,vts,iwat,d,a,b,rho)
            if(verby)then
                if(k.eq.1)then
                    !write(LER,4443)k,t1,nroot1
                else
                    !write(LER,4444)k,t1,nroot1
                endif
            endif
...
c-----
 ...
 2600           continue
                ...
                !write(LER,*) '========================================='
c                if(ilvry.eq.1)
c     1          write(LER,*) 'dc value for LOVE at the lowest period=',t0
c                if(ilvry.eq.2)
c     1          write(LER,*) 'dc value for RAYLEIGH at
c     2                       the lowest period=',t0
c                write(LER,*) ' between c(1)=',cp(1),' and'
c                write(LER,*) '         c(',lmode,')=',  cp(lmode),' :'
c                write(LER,*) ' mean=',dcsum
c                write(LER,*) ' max =',dcmax
c                write(LER,*) ' min =',dcmin
c                write(LER,*) ' '
            endif
 2700       continue
c-----
...
C     1     lmode, kmode, kmodee
            kmodee=kmode
        !call ptshed(1,ifuncc(ilvry),kmodee,t0) !ruoshan,out 2
        ifunc_ptr(rs_count)=ifuncc(ilvry)
        mode_ptr(rs_count)=kmodee
        t_ptr(rs_count)=t0
        if(kmode.gt.0) then
            !call ptsval(1,cp,kmode) !kmode==1 out 3
            cp_ptr(1:kmode,rs_count)=cp(1:kmode)
        endif
        rs_count=rs_count+1
 2800   continue
        !kmodee=-1
        !call ptshed(1,kmodee,kmodee,t0) !out 4
        ifunc_ptr(rs_count)=-1
        mode_ptr(rs_count)=-1
        t_ptr(rs_count)=t0
 3000   continue
        !close(1,status='keep') !fixme ruoshan,fixed
        return
      endsubroutine ! end of disprs
c
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        subroutine poles_sdisp(ifunc,omega0,omega,cmin,cmax,displ,dispr,
     1                        allfluid,mmax,mode,eroot,cphase,nlost,
     2                        index,nroot1,mvts,vts,iwat,d,a,b,rho)
...
c        common/water_sdisp/iwat(NL)
        integer iwat(NL)
c        common/pard_sdisp/ twopi,displ,dispr commented by Ping
c        common/stor/ eroot(MAXMOD),cphase(MAXMOD),nlost,index,nroot1 ! commented by Ping
        double precision eroot(MAXMOD),cphase(MAXMOD)
c        double precision eroot(MAXMOD),cphase(MAXMOD)
        integer nlost,index,nroot1
c        common/vels_sdisp/ mvts(2),vts
        integer mvts(2)
        real*4 vts(NL2,2)
        ...
        logical allfluid ! added by Ping
        twopi=6.283185307179586d+00
...
c-----
        nroot = 0
        c2 = wvmx
        del2 = dltar_sdisp(c2,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
        lyr=1
        jj=1
        do 500 i=1,nmx
...
c-----
   ...
   90       continue
            ...
            do 400 k=1,kk
                ...
                del1 = dltar_sdisp(c1,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
...
c-----
                do 200 ii=1,15
                    c3 = 0.5*(c1+c4)
                   del3 = dltar_sdisp(c3,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
                    if(dsign(1.0d+00,del1)
...
c-----
     ...
     1                     d,a,b,rho)
            del2=dltar_sdisp(c2,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
            del4=dltar_sdisp(c4,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
...
c       write(LOT,*)'eroot=',eroot(i-1),'  dk0=',dk0
            ...
            if(c20.lt.c3.and.c3.le.c40) then
                del2=dltar_sdisp(c20,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
                del3=dltar_sdisp(c3,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
...
c       write(LOT,*)'c3 =',c3 ,'  del3=',del3
     ...
     1                     d,a,b,rho)
                del3=dltar_sdisp(c3,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
                del4=dltar_sdisp(c40,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
c       write(LOT,*)'c20=',c20,'  del2=',del2
        ...
        if (mtest.eq.1) then
            ...
            if ((jjdir0*jjdir1.ne.0).and.(dire1*dire0.lt.0)) then
                ...
                do 626 kk=1,10
                    ...
                    dU1=dltar_sdisp(cU1,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
                    dU2=dltar_sdisp(cU2,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)

                    cD1=(ckeep1*2+ckeep2)/3
                    cD2=(ckeep1+ckeep2*2)/3
                    dD1=dltar_sdisp(cD1,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
                    dD2=dltar_sdisp(cD2,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
               ...
               dkeep1=dltar_sdisp(ckeep1,omega,ifunc,allfluid,mmax,iwat,
     1                     d,a,b,rho)
                    else
                        ckeep0=cU1
                        dkeep0=dU1
...
c-----
  ...
  100   continue
      ...
      subroutine sphere_sdisp(ifunc,mmax,d,a,b,rho)
c-----
...
c        common/water_sdisp/iwat(NL)!commented by Ping
   ...
   10   continue
        ...
        subroutine gcmdln_sdisp(verby,cmin,cmax) !dont need this when
                                                 !merge!! ruoshan
        logical verby
        character names*40
        real*4 cmin,cmax
...
c            call mgtarg(i,names) commented by Ping
            call getarg(i,names) !added by Ping
            if(names(1:2).eq.'-v')then
                verby = .true.
c----
c      fudge to force the dispsersion search to start at a specific value
c-----
...
c                call mgtarg(i,names) commented by Ping
 ...
 1100   continue
        ...
        subroutine usage_sdisp(str) !dont need this too,ruoshan
        character str*(*)
        integer LER
        parameter(LER=0)
        write(LER,*)'usage:',str
...
c-----
        ...
        subroutine varsh_sdisp(h,rsh,lshimag,cossh,rsinsh,sinshr,ex)
        implicit none
        real h
        double precision rsh
        logical lshimag
        ...
        subroutine hskl_sdisp(cossh,rsinsh,sinshr,mu,iwat,hl,exm,exe)
c-----
...
c-----
        ...
        subroutine gtesh_sdisp(esh,einvsh,rsh,wvno,mu,lshimag)
        complex*16 esh(2,2), einvsh(2,2)
...
c-----
        ...
        subroutine gtegsh_sdisp(m,wvno,omega,rsh,lshimag,iwat,b)
        implicit none
...
c        common/model_sdisp/ d,a,b,rho
        real*4 b(NL)
c        real qa, qb, etap, etas, frefp, frefs
c        common/water_sdisp/iwat(NL) ! commented by Ping
        integer iwat(NL)
...
c-----
        ...
        function dltar4_sdisp(wvno,omga,allfluid,mmax,iwat,d,a,b,rho)
c-----
...
c-----
        wvno2=wvno*wvno
        om2 = omga*omga
        call evalg_sdisp(0,mmax,mmax-1,gbr,1,
     1      wvno,omga,om2,wvno2,allfluid,d,a,b,rho)
...
c-----
        ...
        do 500 m = mmm1,1,-1
                ...
                p = rp  * dble(d(m))
                q = rsv * dble(d(m))
                call varsv_sdisp(p,q,rp, rsv, 
     1              cosp, cossv, rsinp, rsinsv, 
     1              sinpr, sinsvr, pex,svex,iwat(m))
                call dnka_sdisp(CA,cosp,rsinp,sinpr,cossv,rsinsv,sinsvr,
...
C        WRITE(6,'(i4,5e11.3)')m,e
  500   continue
        ...
        subroutine dnka_sdisp(CA,cosp,rsinp,sinpr,cossv,rsinsv,sinsvr,
...
c----
     ...
     1          dcmplx(2.0d+00,0.0d+00)
        ...
        subroutine evalg_sdisp(jbdry,m,m1,gbr,in,
     1      wvno,om,om2,wvno2,allfluid,d,a,b,rho)
...
c        common/shwave/bsh(NL), rhosh(NL), qbsh(NL)
        real bsh(NL), rhosh(NL), qbsh(NL)
c        common/modlly/mmax 
c        common/damp/alpha,ieqex
...
c                gbl(in,1) =  dble(rhosh(m))*(dble(bsh(m)*bsh(m))
c     1                  )*rb
...
c-----
        ...
        subroutine varsv_sdisp(p,q, rp, rsv, 
...
c-----
        ...
        subroutine normc_sdisp(ee,ex,nmat)
